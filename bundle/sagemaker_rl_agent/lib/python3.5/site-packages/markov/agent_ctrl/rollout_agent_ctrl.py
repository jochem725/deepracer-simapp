...
class RolloutCtrl(AgentCtrlInterface, ObserverInterface):
    ...
    def _get_car_start_model_state(self, start_dist):
        ...
        prev_wp_idx, next_wp_idx = self._track_data_.center_line_.find_prev_next_waypoints(start_dist, normalized=False)
        
        target_x, target_y = self._reward_.compute_respawn_target(self._track_data_._inner_border_.coords, self._track_data_.get_way_pnts(), self._track_data_._outer_border_.coords, next_wp_idx, self._reverse_dir_)
        target_position = Point([target_x, target_y])

        # Compute the start pose
        start_pose = self._track_data_.center_line_.interpolate_pose(start_dist, finite_difference=FiniteDifference.FORWARD_DIFFERENCE)
