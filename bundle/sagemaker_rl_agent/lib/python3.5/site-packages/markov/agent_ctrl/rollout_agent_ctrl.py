...
class RolloutCtrl(AgentCtrlInterface, ObserverInterface):
    ...
    def _get_car_start_model_state(self, start_dist):
        ...
        prev_wp_idx, next_wp_idx = self._track_data_.center_line.find_prev_next_waypoints(start_dist, normalized=False)
        
        target_x, target_y = self._reward_.compute_respawn_target(self._track_data_.inner_border.coords, self._track_data_.get_way_pnts(), self._track_data_.outer_border.coords, next_wp_idx, self._track_data_.reverse_dir)
        target_position = Point([target_x, target_y])

        # Compute the start pose
        start_pose = self._track_data_.center_line.interpolate_pose(start_dist, finite_difference=FiniteDifference.FORWARD_DIFFERENCE)

        # Update the yaw to point to the target position
        yaw = math.atan2(target_y - start_pose.position.y,
